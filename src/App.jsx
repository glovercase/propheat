// ════════════════════════════════════════════════════════════════
//  PropHeat NZ  –  Property Market Intelligence
// ════════════════════════════════════════════════════════════════
...
const REGION_CONFIG = {
  christchurch: {
    ...
    zoom:    12,
    bargain: { maxValue: 700_000, minLand: 1_000, maxDist: 1 },
  },
  'queenstown-lakes': {
    label:   'Queenstown Lakes',
    center:  [-44.8, 168.9],
    zoom:    10,
    bargain: { maxValue: 1_000_000, minLand: 800, maxDist: 1.5 },
  },
  'all-nz': {
    label:   'All NZ',
...
const SUBURBS = [
  { name: 'Fendalton',    lat: -43.5201, lng: 172.5982, region: 'christchurch', city: 'Christchurch' },
  { name: 'Merivale',     lat: -43.5089, lng: 172.6234, region: 'christchurch', city: 'Christchurch' },
  { name: 'Riccarton',    lat: -43.5356, lng: 172.5993, region: 'christchurch', city: 'Christchurch' },
  { name: 'Addington',    lat: -43.5449, lng: 172.6119, region: 'christchurch', city: 'Christchurch' },
  { name: 'Spreydon',     lat: -43.5556, lng: 172.6070, region: 'christchurch', city: 'Christchurch' },
  { name: 'Sydenham',     lat: -43.5472, lng: 172.6281, region: 'christchurch', city: 'Christchurch' },
  { name: 'St Albans',    lat: -43.5101, lng: 172.6398, region: 'christchurch', city: 'Christchurch' },
  { name: 'Papanui',      lat: -43.4960, lng: 172.6118, region: 'christchurch', city: 'Christchurch' },
  { name: 'Burnside',     lat: -43.5050, lng: 172.5820, region: 'christchurch', city: 'Christchurch' },
  { name: 'Shirley',      lat: -43.5002, lng: 172.6519, region: 'christchurch', city: 'Christchurch' },
  // Coastal: centroid moved inland + hard bounds
  { name: 'New Brighton', lat: -43.5019, lng: 172.715,  maxLng: 172.733, region: 'christchurch', city: 'Christchurch' },
  { name: 'Sumner',       lat: -43.5775, lng: 172.748,  maxLng: 172.763, maxLat: -43.571, region: 'christchurch', city: 'Christchurch' },
  { name: 'Cashmere',     lat: -43.5701, lng: 172.6475, region: 'christchurch', city: 'Christchurch' },
  { name: 'Halswell',     lat: -43.5870, lng: 172.5856, region: 'christchurch', city: 'Christchurch' },
  { name: 'Wigram',       lat: -43.5639, lng: 172.5784, region: 'christchurch', city: 'Christchurch' },
  { name: 'Hornby',       lat: -43.5660, lng: 172.5484, region: 'christchurch', city: 'Christchurch' },
  { name: 'Belfast',      lat: -43.4637, lng: 172.6221, region: 'christchurch', city: 'Christchurch' },
]

// Queenstown Lakes suburbs — bounds prevent lake/hillside placement
const QT_SUBURBS = [
  { name: 'Queenstown',      lat: -45.026,  lng: 168.659,  minLat: -45.037, region: 'queenstown-lakes', city: 'Queenstown' },
  { name: 'Frankton',        lat: -45.017,  lng: 168.736,  minLat: -45.026, region: 'queenstown-lakes', city: 'Queenstown' },
  { name: 'Kelvin Heights',  lat: -45.049,  lng: 168.677,  minLat: -45.061, maxLat: -45.037, minLng: 168.660, maxLng: 168.693, region: 'queenstown-lakes', city: 'Queenstown' },
  { name: 'Queenstown Hill', lat: -45.018,  lng: 168.678,  minLat: -45.030, region: 'queenstown-lakes', city: 'Queenstown' },
  { name: 'Arrowtown',       lat: -44.9435, lng: 168.8296, region: 'queenstown-lakes', city: 'Arrowtown' },
  { name: 'Wanaka',          lat: -44.714,  lng: 169.131,  maxLat: -44.695, region: 'queenstown-lakes', city: 'Wanaka' },
  { name: 'Albert Town',     lat: -44.688,  lng: 169.157,  maxLat: -44.678, region: 'queenstown-lakes', city: 'Wanaka' },
  { name: 'Hawea',           lat: -44.611,  lng: 169.237,  maxLat: -44.600, minLng: 169.228, region: 'queenstown-lakes', city: 'Wanaka' },
]

const ALL_SUBURB_DEFS    = [...SUBURBS, ...QT_SUBURBS]
const ALL_SUBURB_BY_NAME = Object.fromEntries(ALL_SUBURB_DEFS.map(s => [s.name, s]))

// ─────────────────────────────────────────────────────────────────
// STREET NAMES / SCHOOL ZONES
// ─────────────────────────────────────────────────────────────────

const CHCH_STREET_NAMES = [
...
const QT_STREET_NAMES = [
  'Shotover St', 'Beach St', 'Ballarat St', 'Rees St', 'Camp St',
  'Marine Parade', 'Lake Esplanade', 'Peninsula Rd', 'Frankton Rd',
  'Lake Hayes Rd', 'Arrow Lane', 'Buckingham St', 'Berkshire St',
  'Wanaka-Mt Aspiring Rd', 'Ardmore St', 'Lakeside Rd', 'Brownston St',
  'Helwick St', 'McDougall St', 'Cardrona Valley Rd', 'Aubrey Rd',
  'Waimea Tce', 'Orchard Rd', 'Plantation Rd', 'Riverbank Rd',
]

const CHCH_SCHOOL_ZONES = [
...
const QT_SCHOOL_ZONES = [
  'Queenstown Primary', 'Arrowtown School', 'Wanaka Primary',
  "St Joseph's", 'Mount Aspiring College', 'Wakatipu High School',
  'Frankton School', 'Lake Hayes School',
]
...
const PROP_TYPES = ['house+land', 'land', 'residential', 'new build', 'lifestyle', 'apartment']

const CHCH_PROP_TYPE_CDF = [0.43, 0.55, 0.83, 1.00]
const CHCH_PROP_TYPE_MAP = ['residential', 'land', 'house+land', 'new build']
const QT_PROP_TYPE_CDF   = [0.30, 0.45, 0.62, 0.78, 1.00]
...
function generateProperties() {
  ...
  const clamp   = (v, mn, mx) => Math.max(mn ?? -Infinity, Math.min(mx ?? Infinity, v))
  ...
  // ── 498 Christchurch properties ───────────────────────────────
  for (let i = 2; i < 500; i++) {
    const suburb   = pick(SUBURBS)
    const propType = pickCdf(CHCH_PROP_TYPE_CDF, CHCH_PROP_TYPE_MAP)
    ...
    props.push({
      id: `p${i}`,
      address: `${ri(1, 250)} ${pick(CHCH_STREET_NAMES)}`,
      suburb: suburb.name,
      ...
      schoolZone:     pick(CHCH_SCHOOL_ZONES),
  ...
  // ── 80 Queenstown Lakes properties ────────────────────────────
  for (let i = 500; i < 580; i++) {
    const suburb   = pick(QT_SUBURBS)
    const propType = pickCdf(QT_PROP_TYPE_CDF, QT_PROP_TYPE_MAP)
    const isLand   = propType === 'land'
    ...
    props.push({
      id: `qt${i}`,
      address: `${ri(1, 180)} ${pick(QT_STREET_NAMES)}`,
      suburb: suburb.name,
      lat: clamp(rawLat, suburb.minLat, suburb.maxLat),
      lng: clamp(rawLng, suburb.minLng, suburb.maxLng),
      ...
      watchlistCount: ri(0, 200),
      yearBuilt:      ri(1970, 2024),
      schoolZone:     pick(QT_SCHOOL_ZONES),
      nearPublicTransport: rand() > 0.45,
      nearPark:            rand() > 0.30,
      distanceToCityCentreKm: parseFloat(
...
function RegionSelector({ selected, onChange }) {
  const options = [
    { id: 'christchurch',     label: 'Christchurch' },
    { id: 'queenstown-lakes', label: 'Queenstown Lakes' },
...
// ── Area multi-select (cities or suburbs) ─────────────────────────
function AreaMultiSelect({ selected, onChange, availableItems, placeholder = 'All' }) {
...
function FilterSidebar({ filters, onChange, open, onToggle, availableCities, availableSuburbs }) {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ opacity: open ? 1 : 0, transition: 'opacity 0.2s ease', pointerEvents: open ? 'auto' : 'none' }}
      >
        <div className="p-4" style={{ width: '240px' }}>
          ...
          <div className="mb-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-orange-600">Hot Filters</span>
            </div>
            <div className="flex flex-col gap-4">
              ...
              />
              <RangeSlider
                label="Min page views" min={0} max={5000} step={50} value={filters.minPageViews}
                format={v => v.toLocaleString()} onChange={set('minPageViews')} variant="hot"
              />
              <RangeSlider
                label="Min watchlist count" min={0} max={200} value={filters.minWatchlistCount}
                onChange={set('minWatchlistCount')} variant="hot"
              />
              ...
              <div>
                ...
                <AreaMultiSelect
                  ...
                  availableItems={availableSuburbs}
          ...
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest text-emerald-600">Bargain Filters</span>
            </div>
            <div className="flex flex-col gap-4">
              <RangeSlider
                label="Max estimated value" min={200000} max={1500000} step={25000}
                value={filters.maxEstimatedValue} format={fmtNZD}
                onChange={set('maxEstimatedValue')} variant="bargain"
              />
              <RangeSlider
                label="Min land size" min={200} max={3000} step={50} value={filters.minLandSize}
              ...
              <RangeSlider
                label="Max distance to hotspot" min={0.5} max={5} step={0.5}
                value={filters.maxDistanceToHotspot} format={v => `${v}km`}
                onChange={set('maxDistanceToHotspot')} variant="bargain"
              />
            </div>
          </div>
...
export default function App() {
  const [selectedRegion,    setSelectedRegion]    = useState('christchurch')
  ...
  const handleRegionChange = useCallback(region => {
    ...
    setFilters(prev => ({
      ...
      selectedSuburbs:      [],
      selectedCities:       [],
      maxEstimatedValue:    cfg.bargain.maxValue,
      minLandSize:          cfg.bargain.minLand,
      maxDistanceToHotspot: cfg.bargain.maxDist,
    }))
    if (mapRef.current) {
      mapRef.current.flyTo(cfg.center, cfg.zoom, { duration: 1.5, easeLinearity: 0.3 })
    }
  }, [])

  // ── Derived suburb / city lists ───────────────────────────────

  const availableCities = useMemo(() => {
    if (selectedRegion === 'christchurch')     return ['Christchurch']
    if (selectedRegion === 'queenstown-lakes') return ['Queenstown', 'Arrowtown', 'Wanaka']
    return ['Christchurch', 'Queenstown', 'Arrowtown', 'Wanaka']
  }, [selectedRegion])

  const availableSuburbs = useMemo(() => {
    let defs = selectedRegion === 'all-nz'
      ? ALL_SUBURB_DEFS
      : ALL_SUBURB_DEFS.filter(s => s.region === selectedRegion)
    if (filters.selectedCities.length > 0)
      defs = defs.filter(s => filters.selectedCities.includes(s.city))
    return defs.map(s => s.name)
  }, [selectedRegion, filters.selectedCities])

  // ── Derived data ──────────────────────────────────────────────

  const filteredProperties = useMemo(() => {
    return ALL_PROPERTIES.filter(p => {
      if (selectedRegion !== 'all-nz' && p.region !== selectedRegion)               return false
      if (filters.selectedCities.length  > 0 && !filters.selectedCities.includes(p.city))    return false
    ...
    selectedRegion,
    filters.selectedCities, filters.selectedSuburbs,
    filters.maxDaysOnMarket, filters.minPageViews, filters.minWatchlistCount,
  ])
  ...
  const bargains = useMemo(() => {
    ...
    const pool = selectedRegion === 'all-nz'
      ? ALL_PROPERTIES
      : ALL_PROPERTIES.filter(p => p.region === selectedRegion)
    ...
    selectedRegion,
    filters.maxEstimatedValue, filters.minLandSize,
    filters.selectedPropTypes, filters.maxDistanceToHotspot,
    hotSuburbs,
  ])
  ...
  const stats = useMemo(() => {
    ...
    return { hotCount, bargainCount: bargains.length, hottestSuburb: hottestSuburb?.name ?? '—' }
  ...
  useEffect(() => {
    ...
    const cfg = REGION_CONFIG['christchurch']
    const map = L.map(mapContainerRef.current, { center: cfg.center, zoom: cfg.zoom, zoomControl: true })
    L.tileLayer(
  ...
  useEffect(() => {
    ...
    for (const prop of filteredProperties) {
      ...
      const icon  = L.divIcon({
        className: '',
        html: `<div style="width:10px;height:10px;border-radius:50%;background:${color};border:2px solid rgba(255,255,255,0.9);box-shadow:0 1px 4px rgba(0,0,0,0.25),0 0 0 1px ${color}44;"></div>`,
  ...
  useEffect(() => {
    ...
    for (const prop of bargains) {
      const icon = L.divIcon({
        className: '',
        html: `<div style="width:24px;height:24px;border-radius:50%;background:#fff;border:2px solid #16a34a;box-shadow:0 2px 8px rgba(22,163,74,0.35),0 1px 3px rgba(0,0,0,0.12);display:flex;align-items:center;justify-content:center;font-size:13px;cursor:pointer;">⭐</div>`,
  ...
  return (
    <div className="h-screen flex flex-col bg-slate-50 overflow-hidden">
      ...
      <header className="shrink-0 flex items-center justify-between gap-4 px-5 py-2.5 bg-white border-b border-slate-200 shadow-sm">
        ...
        {/* Region selector */}
        <RegionSelector selected={selectedRegion} onChange={handleRegionChange} />
